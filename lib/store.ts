...
export const useArena = create<Store>()(persist((set,get)=>({
 ...
 answer:(c,reply)=>{const s=get().session;if(!s||s.ended||s.turns.length>=4)return; const trust=outcome(s.config,[...s.turns,{...c,reply:''}]).trust;const finalReply=reply?.trim().slice(0,800)||respond(s.config,s.turns.length,c,trust);set({session:{...s,draft:'',turns:[...s.turns,{...c,reply:finalReply}]}});},
